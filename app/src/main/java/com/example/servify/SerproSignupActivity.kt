...
class SerproSignupActivity : AppCompatActivity() {
    ...
    private fun saveUserDataToFirebase(encodedImage: String, progressDialog: ProgressDialog) {
        ...
        val user = User1(name, email, mobile, address, password, encodedImage,business_title,business_description)
    ...
    private fun validateInputs(): Boolean {
        ...
        if(business_title.isEmpty() || business_title.length<5){
            binding.etBusinessName.error="Business Name At Least of 5 chars"
            return false
        }
        if(business_description.isEmpty() || business_description.length<5){
            binding.etBusinessName.error="Business Description At Least of 5 chars"
            return false;
        }
